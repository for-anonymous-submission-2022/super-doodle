...
class NDRC_Test:
    def test(model, dataloader, device, pair, label_reversed_dict):
        ...
        for i, batch in enumerate(dataloader):
            ...
            for idx, label in enumerate(true_labels):
                if label != -100:
                    ...
                    reversed_pair_label.append(label_reversed_dict[true_labels_pair[idx]])
            if len(reversed_true_label) > 0:
                reversed_true_labels.append(reversed_true_label)
                reversed_pred_labels.append(reversed_pred_label)
                reversed_pair_labels.append(reversed_pair_label)
        ...
        if pair:
            return reversed_true_labels, reversed_pair_labels, reversed_pred_labels
