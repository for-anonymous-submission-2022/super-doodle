...
from dataset  import NDRC_Dataset
...
train_dataset =\
 NDRC_Dataset(path=args.train_path, 
...
# Create pytorch dataset.
valid_dataset =\
 NDRC_Dataset(path=args.valid_path, 
                    use_tokenizer=tokenizer, 
                    sent_col = 'sent_tokens',
                    sent_tags_col = 'sent_tags',
                    labels_ids =labels_ids,
...
checkpoint_name = 'checkpoint/' + 'NDRC' + str(args.train_path.split('/')[3]) +'_' +  str(args.learning_rate) + '_' +  args.model_name_or_path.split('/')[-1] + '_' + args.train_path.split('/')[-2] + '.pt'  
