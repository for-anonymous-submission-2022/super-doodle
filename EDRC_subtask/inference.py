...
class Prediction:
    ...
    def run(self, stage2_processed):
        ...
        with torch.no_grad():
            ...
            self.output_EDSC = ids_to_labels[prediction[0]]
        
        return self.input_EDSC, self.output_EDSC
    
    def process_inputs(self, remain):
        self.input_EDSC = ' '.join(remain)
        inputs = self.tokenizer(
            self.input_EDSC, add_special_tokens=True, truncation=True, padding=True, return_tensors='pt', max_length=256
            )
